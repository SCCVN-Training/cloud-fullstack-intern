...
resource "aws_secretsmanager_secret_version" "app_initial" {
  ...
  secret_string = jsonencode({
    DATABASE_URL         = "postgresql://${var.db_username}:${jsondecode(data.aws_secretsmanager_secret_version.rds_secret_version.secret_string)["password"]}@${aws_db_instance.postgres.endpoint}/${var.project_name}?sslmode=require"
