...
async def get_seasonal_anime(
    ...
    """
    logger.info(f"[Seasonal] Request started - user: {current_user.id}, "
    ...
    # ✅ Rate limit check
    logger.debug(f"[Seasonal] Checking rate limit for user: {current_user.id}")
    allowed, remaining, retry_after = await rate_limiter.check_seasonal(
        user_id=str(current_user.id)
    )

    if not allowed:
        logger.warning(f"[Seasonal] Rate limit exceeded for user: {current_user.id}")
        raise HTTPException(
...
async def search_anime(
    ...
    ✅ Subsequent requests: Lightning fast from cache (< 100ms)
    """
    logger.info(f"[Search] Request started - user: {current_user.id}, "
                f"query: '{query}', page: {page}, per_page: {per_page}")

    # ✅ Rate limit check
    logger.debug(f"[Search] Checking rate limit for user: {current_user.id}")
    allowed, remaining, retry_after = await rate_limiter.check_search(
        user_id=str(current_user.id)
    )

    if not allowed:
        logger.warning(f"[Search] Rate limit exceeded for user: {current_user.id}")
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
...
#     logger.info(f"[Admin] Refreshing seasonal cache for {season} {year} by user: {current_user.id}")
...
#     logger.info(f"[Admin] Cache stats requested by user: {current_user.id}")
#     return stats
