UPSERT_USER_SHARE_FILE = """
    INSERT INTO storage.acl_entries (file_id, principal_type, grantee_id, permission, created_by, password_hash)
    VALUES ($1, 'user', $2, $3::storage.permission_level, $4, $5)
...
UPSERT_USER_SHARE_FOLDER = """
    INSERT INTO storage.acl_entries (folder_id, principal_type, grantee_id, permission, created_by, password_hash)
    VALUES ($1, 'user', $2, $3::storage.permission_level, $4, $5)
...
UPSERT_PUBLIC_LINK_FILE = """
    INSERT INTO storage.acl_entries (file_id, principal_type, share_token, password_hash, permission, created_by)
    VALUES ($1, 'public_link', $2, $3, $4::storage.permission_level, $5)
...
UPSERT_PUBLIC_LINK_FOLDER = """
    INSERT INTO storage.acl_entries (folder_id, principal_type, share_token, password_hash, permission, created_by)
    VALUES ($1, 'public_link', $2, $3, $4::storage.permission_level, $5)
...
REVOKE_PUBLIC_LINK_FILE = """
    ...
    DELETE FROM storage.public_link_visitors
...
CHECK_OWNER_FILE = """
    SELECT 1 FROM storage.files WHERE id = $1 AND owner_id = $2 AND is_trashed = FALSE
"""

CHECK_OWNER_FOLDER = """
    SELECT 1 FROM storage.folders WHERE id = $1 AND owner_id = $2 AND is_trashed = FALSE
"""
...
GET_ACL_BY_TOKEN = """
    ...
    WHERE a.share_token = $1 AND a.revoked_at IS NULL
"""

UPSERT_PUBLIC_LINK_VISITOR = """
    INSERT INTO storage.public_link_visitors (user_id, acl_entry_id, last_accessed_at)
    VALUES ($1, $2, NOW())
