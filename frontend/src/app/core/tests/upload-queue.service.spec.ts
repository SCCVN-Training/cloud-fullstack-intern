...
describe('UploadQueueService', () => {
  ...
  it('should compute total progress percentage correctly', () => {
    service.queue.set([
      { id: '1', sizeBytes: 100, uploadedBytes: 50, status: 'uploading' } as any,
      { id: '2', sizeBytes: 100, uploadedBytes: 100, status: 'completed' } as any,
    ]);
  ...
  it('should cancel upload and remove from queue', () => {
    service.queue.set([
      { id: '1', sizeBytes: 100, uploadedBytes: 50, status: 'uploading', uploadId: 'u1', storageKey: 'k1' } as any,
    ]);

    service.cancelUpload('1');
    expect(service.queue().length).toBe(0);
    expect(mockFileService.abortMultipartUpload).toHaveBeenCalledWith({ upload_id: 'u1', storage_key: 'k1' });
  });
